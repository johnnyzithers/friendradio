...
http.createServer(function (req, res) {
	...
	if (uri == '/player.html') {
		// Set CORS headers
		console.log("http://localhost:8000/output1.m3u8");
		...
		// res.write('<video id="video"></video>');


		// res.write('var video = document.getElementById(\'video\');');
		// res.write('if(Hls.isSupported())');
		// res.write('{');
		// res.write('console.log(document.location.pathname);');
		// res.write('var hls = new Hls();');
		// res.write('hls.loadSource(\'http://localhost:8000/output1.m3u8\');');
		// res.write('hls.attachMedia(video);');
		// res.write('hls.on(Hls.Events.MANIFEST_PARSED,function() {');
		// res.write('video.play();});}</body></html>');
